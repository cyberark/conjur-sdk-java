...
docker run \
    ...
    --workdir "${PWD}" \
    tools \
        mvn --batch-mode -f ${PWD}/client/pom.xml versions:set -DnewVersion="$(<VERSION)" -Dmaven.test.skip -DargLine="--add-opens java.base/sun.nio.ch=ALL-UNNAMED --add-opens java.base/java.io=ALL-UNNAMED"
...
docker run \
    ...
    --workdir "${PWD}" \
    tools \
        mvn --batch-mode -f ${PWD}/client/pom.xml package -Dmaven.test.skip -DargLine='--add-opens java.base/sun.nio.ch=ALL-UNNAMED --add-opens java.base/java.io=ALL-UNNAMED'
...
docker run \
    ...
    --workdir "${PWD}" \
    tools \
        find ./client/target -type f -name 'conjur-sdk-java-*-SNAPSHOT.jar'
