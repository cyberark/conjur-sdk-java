# CyberArk Secrets Manager Generated Java Client
An API client for CyberArk Secrets Manager written in Java and generated by the [OpenApi](https://github.com/cyberark/conjur-openapi-spec) specification
...
and is trusted to use with Conjur OSS, Secrets Manager, Self-Hosted, and Secrets Manager, SaaS****. For more detailed information on our certification levels, see [our community guidelines](https://github.com/cyberark/community/blob/master/Conjur/conventions/certification-levels.md#trusted).

Disclaimer: Please note that OIDC and LDAP are not currently supported for Secrets Manager, SaaS. Please refer to the Secrets Manager, SaaS documentation for more information on supported authentication methods.
https://docs.cyberark.com/secrets-manager-saas/latest/en/content/resources/_topnav/cc_home.htm
...
| Java Client Version | [Secrets Manager API Version](https://github.com/cyberark/conjur-openapi-spec/releases) |
| :-----------------: | :----------------: |
| 4.0.x               | 5.1.1              |
| 4.1.x               | 5.3.0              |
...
It is assumed that Conjur OSS, Secrets Manager, Self-Hosted, or Secrets Manager, SaaS and the Secrets Manager CLI have already been
installed in the environment and running in the background. If you haven't done so,
follow these instructions for installation of [Conjur OSS](https://www.conjur.org/get-started/quick-start/oss-environment/)
and these for installation of [Secrets Manager, Self-Hosted](https://docs.cyberark.com/conjur-enterprise/latest/en/content/hometileslps/lp-tile2.htm).
...
### Setup
The Secrets Manager Java SDK can be imported manually through building the source code locally, 
...
these require access to a Secrets Manager instance. You can run the integration tests with
...
the Secrets Manager certificate into the client either by way of setting an environment variable
...
for your app to authenticate against a Secrets Manager server. Environment variables are the best
...
can now create a `Conjur` class instance values to access Secrets Manager services and make RESTful API calls.
...
credentials and a proper Secrets Manager policy loaded declaring the secret.
...
```
_Note:_ The ACCOUNT constant is the Secrets Manager account the variable was loaded under.
